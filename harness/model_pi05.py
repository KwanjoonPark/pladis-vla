...
from .model_gr00t import OfficialGr00tPolicy
...
def _identity_chunk(chunk: np.ndarray) -> np.ndarray:
    ...
    OfficialGr00tPolicy.to_env_actions (``g -> sign(2g-1) * -1``) is
...
class OfficialPi05Policy:
    """Official openpi ``Policy`` behind the harness's ModelAdapter interface
    (harness/model_base.py): wrap_obs / predict_chunk / to_env_actions.

    predict_action_batch(env_obs, mode="eval") -> (actions (1, 10, 7), None)
    is kept as the historical interface (verify_pi05_delivery.py and
    eval_arm._assert_pi05_delivery use it).
    """

    name = "pi05"

    # --- the rollout seam (harness/model_base.py). Mandatory attributes, never
    # defaulted: a model that forgets to_env_actions must crash, not silently inherit
    # GR00T's gripper remap. Reusing OfficialGr00tPolicy.wrap_obs verbatim is the
    # point, not laziness — it guarantees π0.5 and GR00T see bit-identical pixels and
    # the same 8-D state, and it already matches what RLinf's standalone
    # libero_eval.py builds for π0.5.
    wrap_obs = staticmethod(OfficialGr00tPolicy.wrap_obs)
    to_env_actions = staticmethod(_identity_chunk)
    ...
    def predict_chunk(self, env_obs: dict) -> np.ndarray:
        ...
        return arr[0] if arr.ndim == 3 else arr  # (10, 7), already env space
    ...
    def predict_action_batch(self, env_obs: dict, mode: str = "eval"):
        ...
        obs = {
            # uint8 HWC, already 180°-rotated by wrap_obs. Do NOT resize to 224
