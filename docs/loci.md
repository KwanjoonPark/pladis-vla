...
Mapping (`pladis/attn_gr00t.py`): each even block is a dedicated
...
| fp32 upcast of softmax/entmax logits | attn_gr00t | numerical stability under bf16 autocast (official runs fp16/fp32 ambient) |
| bool→additive mask with large-finite clamp | attn_gr00t | SDPA bool-mask convention + entmax NaN safety |
| `β` (inverse temperature) exposed as a flag | all hooks | temperature-control arm (paper suppl. G.1) |
| λ=0 delegates to the native attention path | attn_gr00t | official gates the processor swap on `pladis_scale > 0` (pipeline_sdxl.py:1215,1707); base0 ≡ vanilla bit-exact |
